...
apt-get update
apt-get install --download-only --no-install-recommends -y clang lld
mkdir /tmp/packages/
cp -t /tmp/packages /var/cache/apt/archives/*.deb

VERSION=$(dpkg -f /tmp/packages/clang-* Version |  sed -r 's/^1:([[:digit:]]+)~\+\+([[:digit:]]+)\+([a-z0-9]+).*$/\1-\2-\3/')
echo "VERSION = $VERSION"
OUTPUT_NAME="llvm-toolchain-$VERSION.tar.gz"
tar -czvf "$OUTPUT_NAME" /tmp/packages/
